...
class Store extends CommandBaseClass {
  ...
  async run(context) {
    ...
    let page = 1;
    const max_pages = Math.ceil(store_items.length / 5);
        ...
        .then(async collected => {
          ...
          switch (reaction.emoji.name) {
            ...
            case emojis[7]:
              return;
            default: {
              const index = emojis.findIndex(x => x === reaction.emoji.name);
    ...
    async function showItem(item_id) {
      ...
      const fields = await store_item.getFields();
      const options_text = [];
      const options = await store_item.getOptions();
    ...
    async function updateStoreList() {
      ...
      for (let index = 0; index < 5; index++) {
        const item_index = index + 5 * (page - 1);
        if (item_index === store_items.length)
          break;
        const item = store_items[item_index];
        let str = '';
        item_list += (index + 1) + '. ';
        str = item.name.split(':');
