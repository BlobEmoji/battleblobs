...
class BotPlayer extends Player {
  ...
  async blobBuilder(level) {
    const random_level = Math.ceil(Math.random() * Math.sqrt(level) - Math.sqrt(level)) + level;
    const hp_stat = Math.floor((2 * 30 + (Math.random() * 32)) * random_level / 100 + random_level + 10);
    const random_blob = await this.controller.connection.generateRandomBlobDef();
    const moves = await this.controller.connection.generateMoveSet(random_blob);
    const ret_val = {
      vitality: hp_stat,
      health: hp_stat,
