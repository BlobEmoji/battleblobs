class Turn {
  ...
  async useMove(move, turn) {
    const player = this.controller.players[turn];
    const blobs = [player.selected_blob, player.opponent.selected_blob];
    const prefixes = [];
    if (move === null) {
      // struggle
      move = {
    ...
    const attack_ability = await this.processStatusEffects(blobs, prefixes, player);
    ...
    const damage_dealt = Math.ceil(attack_ability.attack_power * ((2 * blobs[0].blob_level / 5 + 2) * move.damage * blobs[0].cur_stats[0] / blobs[1].cur_stats[1]) / 50);
  ...
  async checkGameOver(player) {
    const current_party_fainted = await player.checkPartyFainted();
    const enemy_party_fainted = await player.opponent.checkPartyFainted();
  ...
  async processStatusEffects(blobs, prefixes, player) {
    ...
    const attack_ability = { can_attack: true, attack_power: 1 };
    ...
    for (let i = 0; i < blobs[0].statuses.length; i++) {
      ...
      if (current_status.skip_chance > 0 && current_status.skip_chance > Math.random()) {
        ...
        const dmg = Math.ceil(blobs[0].vitality * current_status.damage_per_turn);
        blobs[0].health -= dmg;
